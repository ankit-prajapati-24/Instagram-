...
def test_choosing_a_track_downloads_it_under_the_plan(settings):
    def handler(request):
        if request.url.path.endswith("/a1/"):
            return httpx.Response(200, json=_track("a1"))
        return httpx.Response(200, content=b"ID3" + b"\0" * 4000)
...
def test_the_choice_remembers_the_credit_it_owes(settings):
    def handler(request):
        if request.url.path.endswith("/a1/"):
...
def test_a_pick_carries_an_id_and_the_url_is_read_back_from_openverse(
    ...
    # The only non-Openverse URL fetched is the one Openverse named.
